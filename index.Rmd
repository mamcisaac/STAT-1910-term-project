...
1. A numerical outcome variable $y$. 
    + Note 1: binary outcomes variables with 0/1 values are not technically numerical; your outcome must have many different realizations for this project to work (e.g., it can't be a count with only 10 or so unique values).
    + Note 2: Avoid [compositional data](https://en.wikipedia.org/wiki/Compositional_data), where different outcomes add up to some known total (e.g., looking at proportion on the hot 100 of different genres of music won't work because the observations are not independent: if one genre has 80% of the top songs, then that tells you that another must have less than 20%). 
